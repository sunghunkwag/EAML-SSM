...
class MetaMAML:
    ...
    def __init__(self, model: nn.Module, inner_lr: float = 0.01, outer_lr: float = 0.001, first_order: bool = False):
    ...
    def adapt_task(
        ...
        fast_weights = OrderedDict((name, param.clone()) for name, param in self.model.named_parameters())
        ...
        for step in range(num_steps):
            ...
            grads = torch.autograd.grad(step_loss, fast_weights.values(), create_graph=not self.first_order)

            fast_weights = OrderedDict(
                (name, param - self.inner_lr * grad) for (name, param), grad in zip(fast_weights.items(), grads)
            )
    ...
    def get_fast_weights(self) -> OrderedDictType[str, torch.Tensor]:
        ...
        return OrderedDict((name, param.clone()) for name, param in self.model.named_parameters())
