...
class Environment:
    ...
    def step(
        ...
        Union[np.ndarray, List[np.ndarray]], Union[float, List[float]], Union[bool, List[bool]], Union[Dict, List[Dict]]
    ]:
    ...
    def get_episode_statistics(self) -> Dict[str, List[float]]:
        ...
        """
        return {"returns": self._episode_returns.copy(), "lengths": self._episode_lengths.copy()}
...
class MetaEnvironment(Environment):
    ...
    def __init__(self, env_name: str = "CartPole-v1", num_tasks: int = 5, batch_size: int = 1, **kwargs):
