...
class TestMetaMAMLStateful:
    ...
    def stateful_model(self):
        ...
        return StateSpaceModel(state_dim=4, input_dim=4, output_dim=1, hidden_dim=8)  # Smaller hidden dim for testing

    @pytest.fixture
    def maml(self, stateful_model):
        """Create MetaMAML instance."""
        return MetaMAML(model=stateful_model, inner_lr=0.01, outer_lr=0.001)
    ...
    def test_meta_update_stateful_sequential(self, maml, stateful_model):
        ...
        assert any(
            not torch.equal(initial_params[name], updated_params[name])
            for name in initial_params
            if updated_params[name].requires_grad
        )
